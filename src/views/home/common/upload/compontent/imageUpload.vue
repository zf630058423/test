  ...
  <div class="simple-upload">
    ...
    >
      ...
      <div slot="tip" class="el-upload__tip">
        只可上传大小不超过10M，格式为png、jpg、jpeg的文件
      </div>
    </el-upload>
    <div class="img-box">
...
export default {
  ...
  mounted() {
    this.imgList = this.curImgList;
    this.disabled = this.curDisabled;
  },
  methods: {
    onSuccessHandler(row) {
      if (row.code === 100) {
        this.imgList.push({ url: row.data });
      }
    },
    deleteImg(index) {
    ...
    uploadChange(file) {
      console.log("file：", file);
      this.uploading = true;
      /* fileApis.uploadFile(file.raw, (data, status) => {
          if (status === "success") {
            this.imgList.push({
              url: ''
            })
