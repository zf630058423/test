...
export default {
  data(){
    return{
      ...
      direction:'next',
      curIndex:0,
      images:[imgTest1,imgTest2,imgTest3,imgTest4,imgTest5,imgTest6,imgTest7,imgTest8],
    }
  },
  computed:{
    curImage(){
...
.next-image-enter-active, .next-image-leave-active, 
.prev-image-enter-active, .prev-image-leave-active{
  transition: .5s;
}

//离开最终的状态
.next-image-enter, .next-image-leave-to,
.prev-image-enter,.prev-image-leave-to{
  opacity: 0;
}
.next-image-enter, .prev-image-leave-to{
  transform: translateX(200px);
}

.next-image-leave-to,.prev-image-enter{
  transform: translateX(-200px);
}
